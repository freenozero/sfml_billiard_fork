...
ThreeBallGame::ThreeBallGame(int width, int height, int fpsLimit, int option)
...
void ThreeBallGame::makeBilliardObject()
{
	// 테스트 코드 변수 초기화
	//sCatchingBall = false;
	//catchedBall = nullptr;
...
ThreeBallGame::~ThreeBallGame(void)
{
	// UI 인스턴스 해제  
	// 게임 오브젝트들 해제
...
sf::Font* ThreeBallGame::font = nullptr;
const sf::Font& ThreeBallGame::getFont(void)
{
	if (font == nullptr) {
		font = new sf::Font;
...
void ThreeBallGame::handle(sf::Event& ev)
{
...
void ThreeBallGame::update(void)
{
	//타이머
	...
	//플레이어 업데이트, 점수판 업데이트
	Player::WhoisTurn().ThreeBallupdate(*playerBall, Velocity);


	//테스트코드
	/*
	//플레이어 볼의 속도조절 후 3쿠션을 만들어버림 
	if (!isCatchingBall && catchedBall != nullptr) {
		playerBall->setVelocity(2, 2);
		playerBall->setCollideBoardCnt(4);
		playerBall->setTrueCB();
		catchedBall = nullptr;
	}
	*/
	/*  파워 관련 기능  */
...
void ThreeBallGame::render(sf::RenderTarget& target)
{
	// 화면 클리어 
...
void ThreeBallGame::renderDirection(sf::RenderTarget& target) {
	if (!playerBall->isSelected()) {
...
void ThreeBallGame::PlayerTimerRender(sf::RenderTarget& target) {
...
sf::Vector2f ThreeBallGame::FixVec(sf::Vector2f vec) {
